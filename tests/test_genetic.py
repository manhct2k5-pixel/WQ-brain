...
class TestGPLearnSimulator(unittest.TestCase):
    ...
    def test_init_population_before_search_tracking(self):
        """Seed programs should load cleanly during simulator initialization."""
        gp = GPLearnSimulator(
            population_size=5,
            random_state=42,
            init_population=[[OPEN, CLOSE, ADD]],
        )

        self.assertEqual(len(gp.population), 1)
        self.assertEqual(len(gp.seen_expressions), 1)
        self.assertIn("add(open,close)", gp.seen_expressions)
    ...
    def test_edge_case_fitness(self):
        ...
        self.assertEqual(fitness1, (float('-inf'), float('-inf')))
        self.assertEqual(fitness2, (float('-inf'), float('-inf')))

    def test_hof_candidate_without_submit_ready_metrics_is_not_auto_seeded(self):
        gp = GPLearnSimulator(random_state=42)
        gp._save_to_initial_population = Mock()

        def hof_only_metric(_expr):
            return {
                "sharpe": 2.1,
                "fitness": 1.7,
                "alpha_score": 58.0,
        ...
        gp.metric = hof_only_metric
        program = Program(
            max_depth=3,
            max_operators=5,
            random_state=np.random.RandomState(42),
            metric=hof_only_metric,
            parimony_coefficient=0.1,
            program=[OPEN, CLOSE, ADD, VOLUME, MUL],
        )

        gp._evaluate_single_program(program)

        self.assertEqual(len(gp.hall_of_fame), 1)
        gp._save_to_initial_population.assert_not_called()

    def test_submit_ready_hof_candidate_is_auto_seeded(self):
        gp = GPLearnSimulator(random_state=42)
        gp._save_to_initial_population = Mock()

        def submit_ready_metric(_expr):
            return {
                "sharpe": 2.1,
                "fitness": 1.8,
                "alpha_score": 78.0,
        ...
        gp.metric = submit_ready_metric
        program = Program(
            max_depth=3,
            max_operators=5,
            random_state=np.random.RandomState(42),
            metric=submit_ready_metric,
            parimony_coefficient=0.1,
            program=[OPEN, CLOSE, ADD, VOLUME, MUL],
        )
        ...
        self.assertEqual(len(gp.hall_of_fame), 1)
        gp._save_to_initial_population.assert_called_once()
