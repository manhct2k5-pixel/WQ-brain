...
class TestProgramUnit(unittest.TestCase):
    ...
    def test_division_unit_rule(self):
        """Division should divide units instead of multiplying them."""
        div_program = [OPEN, VOLUME, DIV]
        div_prog = Program(
            max_depth=3,
            max_operators=5,
            metric=self.metric,
        ...
        units = div_prog.unit
        self.assertEqual(units[0], Int(3))
        self.assertEqual(units[1], Int(2))
        self.assertEqual(units[2], sp.Rational(3, 2))
